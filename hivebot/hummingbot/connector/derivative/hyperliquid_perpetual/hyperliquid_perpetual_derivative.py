...
class HyperliquidPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    def __init__(
        ...
        print("🏗️ DEBUG: Initializing HyperliquidPerpetualDerivative")
        print(f"🔑 DEBUG: API key received: {hyperliquid_perpetual_api_key[:10] + '...' if hyperliquid_perpetual_api_key else 'None'}")
        print(f"🔗 DEBUG: Secret key length: {len(hyperliquid_perpetual_api_secret) if hyperliquid_perpetual_api_secret else 0}")
        print(f"🏦 DEBUG: Use vault: {use_vault}, Trading required: {trading_required}")
    ...
    def cancel(self, trading_pair: str, client_order_id: str):
        """
        Cancel an order using the client order ID.

        :param trading_pair: The market trading pair (e.g. BTC-USDT)
        :param client_order_id: The client order ID to cancel
        """
        self.logger().info(f"🚫 CANCEL REQUESTED: {client_order_id} on {trading_pair}")
        tracked_order = self._order_tracker.fetch_tracked_order(client_order_id)
        if tracked_order is not None:
            self.logger().info(f"✅ Order {client_order_id} found in tracker, sending cancel request")
            safe_ensure_future(self._place_cancel(client_order_id, tracked_order))
        else:
            self.logger().warning(f"❌ Order {client_order_id} not found in tracked orders, cannot cancel")
    ...
    def buy(self,
        ...
        """
        self.logger().info(f"📈 BUY ORDER REQUESTED: {amount} {trading_pair} @ {price}")
    ...
    def sell(self,
        ...
        :return: the id assigned by the connector to the order (the client id)
        """
        self.logger().info(f"📉 SELL ORDER REQUESTED: {amount} {trading_pair} @ {price}")
        order_id = get_new_client_order_id(
            is_buy=False,
            trading_pair=trading_pair,
            hbot_order_id_prefix=self.client_order_id_prefix,
    ...
    async def _place_order(
        ...
        api_key_display = self.hyperliquid_perpetual_api_key[:10] + "..." if self.hyperliquid_perpetual_api_key else "None"
        print(f"🔑 DEBUG: Placing {trade_type.name} order with API key: {api_key_display}")
        print(f"🔐 DEBUG: Use vault: {self._use_vault}, Trading required: {self._trading_required}")
        print(f"🔗 DEBUG: Secret key length: {len(self.hyperliquid_perpetual_secret_key) if self.hyperliquid_perpetual_secret_key else 0}")

        if not self.hyperliquid_perpetual_api_key:
            print("❌ DEBUG: API key is None!")
            raise ValueError("API key is not set - cannot place orders")

        if not self.hyperliquid_perpetual_secret_key:
            print("❌ DEBUG: Secret key is None!")
            raise ValueError("Secret key is not set - cannot place orders")
    ...
    async def _update_balances(self):
        ...
        # **PMM FIX**: For perpetual trading, PMM needs base asset balance to place SELL orders
        # Set a large virtual BTC balance since perpetuals can short without holding the asset
        virtual_btc_balance = Decimal("1000")  # Large enough for any reasonable PMM order size
        self._account_balances["BTC"] = virtual_btc_balance
        self._account_available_balances["BTC"] = virtual_btc_balance
