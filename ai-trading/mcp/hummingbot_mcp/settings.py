...
class Settings(BaseModel):
    ...
    backend_api_base_url: str = Field(default="http://localhost:8001")
    ...
    mcp_port: int = Field(default=8002)
    
    # SSL Configuration
    ssl_enabled: bool = Field(default=False)
    ssl_cert_file: str = Field(default="")
    ssl_key_file: str = Field(default="")
    ssl_ca_file: str = Field(default="")
    ...
    def validate_mcp_transport(cls, v):
        ...
        if v not in valid_transports:
            raise ValueError(f"MCP transport must be one of: {valid_transports}")
        return v
